...
import { deletePrivacyPolicy } from "@/lib/actions/privacy-policies";
import { useRouter, usePathname, useSearchParams } from "next/navigation";
import { useDebounce } from "@/hooks/useDebounce";
import { toast } from "react-hot-toast";
import Link from "next/link";

export function PrivacyPoliciesClient({ privacyPolicies }: { privacyPolicies: any[] }) {
  ...
  const handleOpenDeleteModal = (policy: any) => {
    setSelectedPolicy(policy);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    const res = await deletePrivacyPolicy(selectedPolicy.id);
    setIsDeleting(false);
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4" dir="rtl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">إدارة بنود سياسة الخصوصية والشروط الخاصة بالمتجر وتعديل ترتيبها.</p>
          </div>
        </div>
        <button
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full" dir="rtl">
            <h3 className="text-lg font-bold text-gray-800">بنود سياسة الخصوصية</h3>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>المحتوى (العربية)</TableHead>
                <TableHead>المحتوى (الإنجليزية)</TableHead>
              ...
              {privacyPolicies.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-6 text-gray-500">
                    لا توجد بنود متاحة لسياسة الخصوصية
                  </TableCell>
                </TableRow>
              ) : (
                privacyPolicies.map((policy) => (
                  <TableRow key={policy.id}>
                    <TableCell className="font-bold text-gray-900 max-w-[150px] truncate">{policy.title_ar}</TableCell>
                    <TableCell className="text-gray-700 max-w-[150px] truncate" dir="ltr">{policy.title_en}</TableCell>
                    <TableCell className="text-gray-500 max-w-[200px] truncate">{policy.content_ar}</TableCell>
                    <TableCell className="text-gray-500 max-w-[200px] truncate" dir="ltr">{policy.content_en}</TableCell>
                    <TableCell className="text-gray-700 font-bold">{policy.sort_order ?? 0}</TableCell>
                    <TableCell>
      ...
      <Modal
        ...
        title={selectedPolicy ? "تعديل بند الخصوصية" : "إضافة بند خصوصية جديد"}
      >
        <PrivacyPolicyForm
      ...
      >
        <div className="p-4" dir="rtl">
          <p className="mb-6">
            هل أنت متأكد من رغبتك في حذف البند <strong>{selectedPolicy?.title_ar}</strong>؟ لا يمكن التراجع عن هذا الإجراء.
          </p>
